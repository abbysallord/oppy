    ...
    <div className="space-y-8 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="text-center max-w-3xl mx-auto space-y-3 pt-2">
        ...
        <p className="text-slate-400 text-sm">
          Scraped, filtered, and cached from Unstop, Devpost, RemoteOK, and WeWorkRemotely.
        </p>
      </div>
      ...
      <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-4 sm:p-6 space-y-4 backdrop-blur-md shadow-xl">
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search by title, company, or tech stack (e.g. React, Python, Remote, AI)..."
            className="w-full bg-slate-950/80 border border-slate-800 focus:border-purple-500 rounded-xl pl-12 pr-4 py-3.5 text-sm text-slate-100 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-purple-500/20 transition-all font-sans"
          />
          {search && (
            <button
              onClick={() => setSearch('')}
      ...
      {/* Live Terminal Progress Scan Animation during search */}
      {search.trim() !== '' && (
        <SearchAnimation query={search} />
      )}
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-16 space-y-3">
          ...
          <p className="text-sm text-slate-400 font-medium">Scanning opportunities ledger...</p>
        </div>
        ...
        <div className="bg-slate-900/40 border border-slate-800 rounded-2xl p-12 text-center space-y-3 max-w-md mx-auto">
          ...
          <p className="text-xs text-slate-400">
            Try broadening your search keywords or resetting platform filters.
          </p>
          <button
            onClick={() => {
              setSearch('');
